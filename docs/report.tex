...
\section{Investigated Approaches}
...
\begin{center}
\begin{tabular}{c}
\textbf{Design Document (PDF/DOCX/TXT)} \\
\quad $\downarrow$ Extract raw text \\
\\
\textbf{Semantic Chunker (MiniLM-L6-v2, threshold 0.7)} \\
\quad $\downarrow$ Generate chunks and embeddings \\
\\
\textbf{Document Retriever (Cosine Similarity)} \\
\quad $\downarrow$ Retrieve Top-K most relevant chunks \\
\\
\textbf{Reasoner (Llama 3.1-70B Instruct)} \\
\quad $\downarrow$ Structured JSON response \\
\\
\textbf{Compliance Result (violations, explanations, fixes)} \\
\end{tabular}
\end{center}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{assets/chunking_performance.png.png}
...
\subsubsection{Pull Request Agent}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{assets/workflow-brief.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{assets/design-brief.png}
